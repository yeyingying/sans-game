...
// rules. The first special elite arrives after six 15s choice screens.
{
  ...
  check("无尽第 7/8 轮接格莱德与抱歉怪", C.championForRound(7).championId === "glyde" && C.championForRound(8).championId === "soSorry");
  check("无尽第 9/10 轮接犬神融合体与柠檬面包", C.championForRound(9).championId === "endogeny" && C.championForRound(10).championId === "lemonBread" && C.championForRound(11).championId === "greaterDog");
  ...
  check("真实验室精英在屠杀 4:00 / 4:30 加入", !C.eliteProfilePool(3, 239).some((m) => m.key === "elite_memoryhead") && C.eliteProfilePool(3, 240).some((m) => m.key === "elite_memoryhead") && !C.eliteProfilePool(3, 269).some((m) => m.key === "elite_reaper_bird") && C.eliteProfilePool(3, 270).some((m) => m.key === "elite_reaper_bird"));
