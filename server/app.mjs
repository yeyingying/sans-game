...
for(const [t,c] of [["runs","daily_key"],["scores","daily_key"],["scores","season"],["runs","report"]])try{db.exec(`ALTER TABLE ${t} ADD COLUMN ${c} TEXT NOT NULL DEFAULT ''`)}catch{}
...
function rank(mode,rounds,score,day){return db.prepare(`SELECT 1+COUNT(*) rank FROM scores WHERE mode=? AND season=? ${mode==="daily"?"AND daily_key=?":""} AND (rounds>? OR (rounds=? AND score>?))`).get(...[mode,SEASON,...(mode==="daily"?[day]:[]),rounds,rounds,score]).rank}
...
const server=http.createServer(async(req,res)=>{const origin=ORIGINS.has(req.headers.origin)?req.headers.origin:[...ORIGINS][0];try{
 ...
 const p=session(req); if(u.pathname==="/v1/me"&&req.method==="GET")return p?send(res,200,{player:{nickname:p.nickname,canRenameAt:p.renamed_at+604800000,bound:!!p.recovery_hash}},origin):guest(res,origin);
 if(u.pathname==="/v1/account/restore"&&req.method==="POST"){const key=req.socket.remoteAddress||"unknown",now=Date.now(),attempts=(recoveryAttempts.get(key)||[]).filter(x=>now-x<900000);if(attempts.length>=8)throw err("尝试次数过多，请稍后再试",429);attempts.push(now);recoveryAttempts.set(key,attempts);const b=await read(req),found=stmt.recoverPlayer.get(recoveryHash(b.code));if(!found)throw err("恢复码无效",401);recoveryAttempts.delete(key);return send(res,200,{player:{nickname:found.nickname,bound:true}},origin,{"set-cookie":sessionCookie(found.id)})}
 ...
 if(u.pathname==="/v1/leaderboard"&&req.method==="GET"){const asked=u.searchParams.get("mode"),mode=asked==="endless"?"endless":asked==="daily"?"daily":"normal",ch=u.searchParams.get("character"),df=u.searchParams.get("difficulty"),diff=["0","1","2","3"].includes(df)?Number(df):null,day=mode==="daily"?dailyKey():"";
  const where=`s.mode=? AND s.season=? ${mode==="daily"?"AND s.daily_key=?":""} ${ch?"AND s.character=?":""} ${diff!==null?"AND s.difficulty=?":""}`;
  const args=[mode,SEASON,...(day?[day]:[]),...(ch?[ch]:[]),...(diff!==null?[diff]:[])];
  const rows=db.prepare(`SELECT p.nickname,s.character,s.difficulty,s.score,s.rounds,s.created_at,s.daily_key FROM scores s JOIN players p ON p.id=s.player_id WHERE ${where} ORDER BY s.rounds DESC,s.score DESC,s.created_at ASC LIMIT 100`).all(...args);
  // 你的名次: requester's best entry under the same filters, ranked
  let mine=null;const best=db.prepare(`SELECT s.score,s.rounds,s.created_at FROM scores s WHERE s.player_id=? AND ${where} ORDER BY s.rounds DESC,s.score DESC,s.created_at ASC LIMIT 1`).get(p.id,...args);
  if(best){const better=db.prepare(`SELECT COUNT(*) c FROM scores s WHERE ${where} AND (s.rounds>? OR (s.rounds=? AND s.score>?) OR (s.rounds=? AND s.score=? AND s.created_at<?))`).get(...args,best.rounds,best.rounds,best.score,best.rounds,best.score,best.created_at).c;mine={rank:1+better,score:best.score,rounds:best.rounds}}
