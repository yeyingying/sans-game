...
import { SEASON, dailyKey, expectedScore, isValidCharacter, legacyEndlessStageScore, randomNickname, runProgressError, scoreEntryRunId, settlementScoreEntries, signToken, validateNickname } from "./core.mjs";
import { adminAuthorized, adminBalance, adminCookie, adminOverview, adminPage, clearAdminCookie, deletePlayerData, emptyGuestCutoff, emptyGuestWhere, passwordMatchesHash, uniqueNickname } from "./admin.mjs";
...
const DEPLOY_SHA=process.env.DEPLOY_SHA||(()=>{try{return fs.readFileSync(new URL("./DEPLOY_SHA",import.meta.url),"utf8").trim()}catch{return ""}})();
...
const stmt={
 ...
 score:db.prepare("INSERT OR IGNORE INTO scores(player_id,run_id,mode,character,difficulty,score,rounds,created_at,daily_key,season) VALUES(?,?,?,?,?,?,?,?,?,?)"),
 scoreByRunId:db.prepare("SELECT score FROM scores WHERE run_id=?"),
 stageProgress:db.prepare("UPDATE runs SET last_at=?,last_elapsed=?,last_kills=?,last_rounds=0 WHERE id=?"),
...
// Before dual-board settlement existed, an endless run stored only its
// endless delta. Reconstruct the already server-validated stage score from
// total score minus endless score and restore its missing normal-board row.
for(const old of db.prepare("SELECT s.*,r.report,r.silence FROM scores s JOIN runs r ON r.id=s.run_id WHERE s.mode='endless' AND s.season<>''").all()){
 const stage=legacyEndlessStageScore({report:old.report,endlessScore:old.score,difficulty:old.difficulty,silence:!!old.silence});
 if(stage===null)continue;
 stmt.score.run(old.player_id,scoreEntryRunId(old.run_id,"normal"),"normal",old.character,old.difficulty,stage,0,old.created_at,"",old.season);
}
...
const server=http.createServer(async(req,res)=>{const origin=ORIGINS.has(req.headers.origin)?req.headers.origin:[...ORIGINS][0];try{
 ...
 const u=new URL(req.url,"http://local"); if(u.pathname==="/health")return send(res,200,{ok:true,deploy:DEPLOY_SHA||null,season:SEASON},origin);
 ...
 if(u.pathname.startsWith("/v1/admin/")){
  ...
  if(u.pathname==="/v1/admin/balance"&&req.method==="GET")return send(res,200,adminBalance(db,{days:Number(u.searchParams.get("days")),version:u.searchParams.get("version")||"",character:u.searchParams.get("character")||"",difficulty:u.searchParams.get("difficulty"),device:u.searchParams.get("device")||"",sample:u.searchParams.get("sample")||"first"}),origin);
 ...
 const mr=u.pathname.match(/^\/v1\/runs\/([^/]+)\/report$/); if(mr&&req.method==="POST"){const b=await read(req),r=stmt.run.get(mr[1],p.id);if(!r||signToken(SECRET,String(b.token||""))!==r.token_hash)throw err("无效 run",409);if(r.report)throw err("已上报",409);const s=b.stats&&typeof b.stats==="object"?b.stats:{};const pick={};for(const k of ["version","mode","outcome","elapsed","kills","score","bossReached","bossDefeated","enteredEndless","bossPhaseReached","bossFightSeconds","bossHpPct","hpAtBossPct","damageToBoss","rounds","hpPct","damageTaken","revivesUsed","weapons","contract","speed","metaPower","deathBy","deathSkill","chests","relics","wallet","up"]){const v=s[k];if(v===undefined)continue;pick[k]=Array.isArray(v)?v.slice(0,8).map(x=>String(x).slice(0,32)):typeof v==="boolean"?v:typeof v==="number"&&Number.isFinite(v)?v:String(v).slice(0,32)}stmt.report.run(JSON.stringify(pick),r.id);return send(res,200,{ok:true},origin)}
 const stageClear=u.pathname.match(/^\/v1\/runs\/([^/]+)\/stage-clear$/); if(stageClear&&req.method==="POST"){const b=await read(req),r=stmt.run.get(stageClear[1],p.id);if(!r||r.settled||r.daily_key||signToken(SECRET,String(b.token||""))!==r.token_hash)throw err("无效或已结算的 run",409);const now=Date.now(),elapsed=Number(b.elapsed),kills=Number(b.kills),progressError=runProgressError({elapsed,kills,rounds:0,lastElapsed:r.last_elapsed,lastKills:r.last_kills,lastRounds:r.last_rounds,wallElapsed:(now-r.started_at)/1000});if(progressError){console.warn("stage clear rejected",progressError,r.id);throw err("通关成绩校验失败",422)}const score=expectedScore({kills,elapsed,difficulty:r.difficulty,silence:!!r.silence}),scoreRunId=scoreEntryRunId(r.id,"normal");db.exec("BEGIN IMMEDIATE");try{stmt.stageProgress.run(now,elapsed,kills,r.id);stmt.score.run(p.id,scoreRunId,"normal",r.character,r.difficulty,score,0,now,"",SEASON);db.exec("COMMIT")}catch(e){db.exec("ROLLBACK");throw e}const saved=stmt.scoreByRunId.get(scoreRunId)?.score??score;return send(res,200,{score:saved,rank:rank("normal",p.id,"")},origin)}
 ...
 const stageKills=Number(b.stageKills),stageElapsed=Number(b.stageElapsed);if(!Number.isInteger(stageKills)||stageKills<0||stageKills>kills||!Number.isFinite(stageElapsed)||stageElapsed<0||stageElapsed>elapsed)throw err("结算组成无效",422);const mode=r.daily_key?"daily":b.mode==="endless"?"endless":"normal";if(mode!=="endless"&&rounds!==0)throw err("结算组成无效",422);const total=expectedScore({kills,elapsed,difficulty:r.difficulty,silence:!!r.silence}),stage=expectedScore({kills:stageKills,elapsed:stageElapsed,difficulty:r.difficulty,silence:!!r.silence}),entries=settlementScoreEntries({runId:r.id,mode,totalScore:total,stageScore:stage,rounds});db.exec("BEGIN IMMEDIATE");try{stmt.settled.run(r.id);for(const entry of entries)stmt.score.run(p.id,entry.runId,entry.mode,r.character,r.difficulty,entry.score,entry.rounds,now,entry.mode==="daily"?r.daily_key:"",SEASON);db.exec("COMMIT")}catch(e){db.exec("ROLLBACK");throw e}const finalEntry=entries.find(entry=>entry.mode===mode),score=stmt.scoreByRunId.get(finalEntry.runId)?.score??finalEntry.score;return send(res,200,{score,rank:rank(mode,p.id,r.daily_key),date:r.daily_key||null},origin)}
 if(u.pathname==="/v1/leaderboard"&&req.method==="GET"){const asked=u.searchParams.get("mode"),mode=asked==="endless"?"endless":asked==="daily"?"daily":"normal",ch=u.searchParams.get("character"),df=u.searchParams.get("difficulty"),diff=["0","1","2","3"].includes(df)?Number(df):null,day=mode==="daily"?dailyKey():"";
  ...
  const args=[mode,...(day?[day]:[]),...(ch?[ch]:[]),...(diff!==null?[diff]:[])];
