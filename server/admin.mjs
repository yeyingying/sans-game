...
export function adminOverview(db){
  ...
  const players=db.prepare(`SELECT p.id,p.nickname,p.created_at,p.last_seen_at,p.last_ip_masked,p.last_network_tag,p.last_device,p.last_region,p.last_isp,COUNT(DISTINCT r.id) run_count,COUNT(DISTINCT s.id) score_count,MAX(s.score) best_score,GROUP_CONCAT(DISTINCT r.character) character_list,(SELECT rr.character FROM runs rr WHERE rr.player_id=p.id ORDER BY rr.started_at DESC,rr.id DESC LIMIT 1) last_character FROM players p LEFT JOIN runs r ON r.player_id=p.id LEFT JOIN scores s ON s.player_id=p.id AND s.hidden=0 GROUP BY p.id ORDER BY p.last_seen_at DESC,p.created_at DESC LIMIT 200`).all().map(x=>({...x,characters:playerCharacters(x.character_list,x.last_character)}));
...
const names={sans:'传说之下',ukb:'因果报应',horror:'恐惧传说',hard:'困难模式',insanity:'Insanity'},diff=['普通','狂暴','地狱','屠杀'],mode={normal:'通关',endless:'无尽',daily:'每日'};
