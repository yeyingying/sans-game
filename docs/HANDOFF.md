...
- 放置经济（2026-07-26 二次裁决）：同一角色可以**重复放置**，编队卡不再一次性（卡面显示 ×N 与下一座价格）。免费角色第一座免费；全队按第 N 座 0→1000→10000→100000 梯度计价；付费角色每一座至少其角色价（黑客 15000、精神错乱 10000，不再免费），梯度超过底价后按梯度走（`tdPlaceCost`）。
- 地图为**多条进攻车道**（2-3 条，`map.lanes`）：每条从右缘独立蜿蜒、最后两列并回同一红门；怪物/Boss/无尽首领随机分配车道，速度按所在车道路径长度归一（42 秒行军预算逐车道成立）。旧字段 `map.waypoints/spawn/pathLength` 镜像 0 号车道。
- 塔防镜头锁定：TD 局本体移动输入清零、坐标钉在场中心、`camX` 恒为 0（摇杆也不再绘制）——修复"方向键把视角推出地图、怪物与走廊错位"。
- sans 塔面向当前攻击方向（就近敌人定向，四向走图取帧），不再固定朝左。
- TD 选卡池新增「角色·获得新武器」：给已放塔补一把该角色的可站桩武器（每塔上限 3 把）；专属/品阶/进化卡覆盖塔上全部武器。
- 塔防关闭：访客彩蛋（狗/回声花/饶恕/Temmie/帕子的信）、怪物糖、round 4+ 危险领域。黑客「越界甩掷」在塔防不往左甩（等于送怪进门），往右甩最多到半场且绝不倒拖（weapon.js 读 `world.tdMidX`）。
- 编队第一位是队长。付费角色的天性增伤仍作为全塔团队加成生效，但编队标题、队长卡与局内编队栏都会明确显示“全塔伤害 +15%/+20%”；免费队长会显示“无团队加成”，避免换序后数值暗变。
- 首次进入塔防会显示需点击或按 Enter 确认的教学框；确认前 `elapsed`、刷怪、移动和打门全部冻结。
- 塔防不申请排行榜 run、不上传匿名 run 报告，成绩只保存在当前浏览器的 `best_td`（无尽独立键 `best_endless_td` / `best_endless_round_td`；三个键均带 `PRODUCTION_DEBUG_RUN` 守卫）。
- 位移契约（2026-07-27）：天意级（`isBossLike` = boss/tdBoss）对一切直接改写坐标的位移免疫——锁链拖拽（对 Boss 原地全额结算防死锁）、斧旋捕猎、蓝魂光球、震荡/地刺击退均有守卫；无尽首领可控（与经典一致）。塔防里所有位移武器横向只许向右（甩掷/斧旋掷出并封顶半场），读 `world.tdMidX` 开关，经典行为零变化。
- 破门冲击波不波及天意/无尽首领（否则 Boss 亲手破门＝被白杀＝误触发通关）。塔防不掉宝箱（无本体可捡；经济补偿待裁决）。
- 结算页点空白＝无操作（误触保护）；重开按钮/回车走 `restartSameMode`，塔防回编队页并保留上局编队。
- 角色解锁（2026-07-27 接线修复）：免费角色击杀/Boss 门槛由 `stats.classicKills`（只记经典局击杀，存量迁移既往不咎）与 `bossKills` 判定，`charLocksNow` 统一产出付费+免费锁，选人页/塔防编队/键盘遍历/确认拦截共用；已玩过该角色的存档按最高分豁免。
...
- 手机横屏编队页使用独立大尺寸布局：角色卡 6 列、技能卡 4×2、队伍卡和开始/返回按钮均按至少约 44 物理像素设计。局内 HUD 也分两档（`td.js` 的 `bigUi` = 手机画布或 IS_TOUCH）：编队卡 220×76、入口血条 336×30、计时/经验/信息行字号整体加大；放置态时编队栏上方的信息行切换为黄色指引「放置 XX：点绿色地面 · 再点卡取消」，平时显示队长加成。键盘可用 Enter 进入模式页，方向键/Tab 遍历“角色→技能→队伍→开始”，Enter 添加或移除，Esc 逐层返回；Canvas 带可见焦点、快捷键说明和 `aria-live` 状态播报。
